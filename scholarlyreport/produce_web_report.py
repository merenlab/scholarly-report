    ...
    def _generate_assets(self):
        ...
        function createNetwork(data, containerId) {
            ...
            const simulation = d3.forceSimulation(data.nodes)
                ...
                .force("collision", d3.forceCollide().radius(d => computeNodeRadius(d) + 5));
            ...
            simulation.on("tick", () => {
                ...
                node
                    .attr("cx", d => d.x = Math.max(computeNodeRadius(d), Math.min(width - computeNodeRadius(d), d.x)))
                    .attr("cy", d => d.y = Math.max(computeNodeRadius(d), Math.min(height - computeNodeRadius(d), d.y)));
